...
TEST_F(TimeThisTest, CallbackInvocation)
{
    ...
    auto callback         = [&callback_invoked](const auto& /* duration */) { callback_invoked = true; };
...
TEST_F(TimeThisTest, CallbackReceivesDuration)
{
    ...
    auto callback       = [&duration_valid](const auto& duration) { duration_valid = (duration.count() > 0); };
...
TEST_F(TimeThisTest, CopyOperationsDeleted)
{
    ...
    static_assert(!std::is_copy_constructible_v<siddiqsoft::timethis>, "timethis should not be copy constructible");
    static_assert(!std::is_copy_assignable_v<siddiqsoft::timethis>, "timethis should not be copy assignable");
}
...
TEST_F(TimeThisTest, MoveOperationsDeleted)
{
    ...
    // If this compiles, the test passes
    static_assert(!std::is_move_constructible_v<siddiqsoft::timethis>, "timethis should not be move constructible");
    static_assert(!std::is_move_assignable_v<siddiqsoft::timethis>, "timethis should not be move assignable");
}
...
TEST_F(TimeThisTest, CallbackDurationAccuracy)
{
    ...
    auto callback          = [&received_duration](const auto& duration) { received_duration = duration; };
    ...
    EXPECT_GE(received_ms, 80) << "Callback duration should be at least 80ms";
    EXPECT_LE(received_ms, 5000) << "Callback duration should be reasonable (<5s)";
}
...
TEST_F(TimeThisTest, LongerDuration)
{
    ...
    const auto elapsed_ms = duration_cast<milliseconds>(timer.elapsed()).count();
    EXPECT_GE(elapsed_ms, 200) << "Elapsed time should be at least 200ms";
}
...
TEST_F(TimeThisTest, MultipleIndependentTimers)
{
    ...
    // Sample younger timer2 first, then older timer1, so any scheduling delay
    // widens timer1's elapsed time lead rather than closing it.
    const auto elapsed2 = timer2.elapsed();
    const auto elapsed1 = timer1.elapsed();

    // Timer1 should have more elapsed time than timer2
    EXPECT_GT(elapsed1, elapsed2) << "Timer1 should have more elapsed time";
}
...
TEST_F(TimeThisTest, StreamOperatorIncludesFunctionName)
{
    ...
    EXPECT_NE(output.find("TimeThisTest"), std::string::npos) << "Output should contain test function name";
}
...
TEST_F(TimeThisTest, ToStringIncludesTimestamp)
{
    ...
    EXPECT_NE(output.find("T"), std::string::npos) << "Output should contain ISO timestamp (T separator)";
    EXPECT_NE(output.find("Z"), std::string::npos) << "Output should contain Z (UTC indicator)";
}
...
TEST_F(TimeThisTest, ElapsedIsNoexcept)
{
    ...
    static_assert(noexcept(timer.elapsed()), "elapsed() should be noexcept");
...
TEST_F(TimeThisTest, DestructorIsNoexcept)
{
    ...
    static_assert(std::is_nothrow_destructible_v<siddiqsoft::timethis>, "timethis destructor should be noexcept");
...
TEST_F(TimeThisTest, TypeAliasesCorrect)
{
    ...
    static_assert(std::is_same_v<siddiqsoft::timethis::duration_type, std::chrono::system_clock::duration>,
                  "duration_type should be system_clock::duration");

    static_assert(std::is_same_v<siddiqsoft::timethis::time_point_type, std::chrono::system_clock::time_point>,
...
TEST_F(TimeThisTest, CallbackCalledExactlyOnce)
{
    ...
    auto callback       = [&callback_count](const auto& /* duration */) { callback_count++; };
...
TEST_F(TimeThisTest, LapDefaultUnitIsMicroseconds)
{
    ...
    // Sample the larger unit (ms) first, then default (us) and explicit us,
    // so time progression guarantees lap_us_count >= 1000 * lap_ms_count.
    const auto lap_ms          = timer.lap<milliseconds>();
    const auto lap_us          = timer.lap();
    const auto lap_explicit_us = timer.lap<microseconds>();

    const auto lap_ms_count       = std::stoll(lap_ms);
    const auto lap_us_count       = std::stoll(lap_us);
    const auto lap_explicit_count = std::stoll(lap_explicit_us);

    // Default lap() should match explicit lap<microseconds>() closely
    EXPECT_LE(std::abs(lap_us_count - lap_explicit_count), 500'000) << "Default lap() should match lap<microseconds>()";

    // Microseconds should be approximately 1000x larger than milliseconds
    EXPECT_GT(lap_us_count, lap_ms_count * 500) << "Default lap() should use microseconds (much larger than milliseconds)";
}
...
TEST_F(TimeThisTest, LapWithMilliseconds)
{
    ...
    EXPECT_GE(lap_ms_count, 80) << "lap<milliseconds>() should be at least 80ms";
    EXPECT_LE(lap_ms_count, 5000) << "lap<milliseconds>() should be reasonable (<5s)";
}
...
TEST_F(TimeThisTest, LapWithSeconds)
{
    ...
    siddiqsoft::timethis timer;
    // Verify immediate check returns 0
    const auto lap_s_imm = timer.lap<seconds>();
    EXPECT_EQ(std::stoll(lap_s_imm), 0) << "lap<seconds>() should be 0 immediately after start";
    ...
    // Should be <= 1 second for 100ms sleep
    EXPECT_LE(lap_s_count, 1) << "lap<seconds>() should be <= 1 for 100ms sleep";
}
...
TEST_F(TimeThisTest, LapWithNanoseconds)
{
    ...
    EXPECT_GE(lap_ns_count, 80'000'000) << "lap<nanoseconds>() should be at least 80,000,000ns";
    EXPECT_LE(lap_ns_count, 5'000'000'000) << "lap<nanoseconds>() should be reasonable (<5s)";
}
...
TEST_F(TimeThisTest, LapIsNoexcept)
{
    ...
    static_assert(noexcept(timer.lap()), "lap() should be noexcept");
...
TEST_F(TimeThisTest, LapWithMultipleDurationTypes)
{
    ...
    // Sample from coarsest to finest unit so time progression reinforces unit scale differences
    const auto lap_s  = timer.lap<seconds>();
    const auto lap_ms = timer.lap<milliseconds>();
    ...
    EXPECT_GE(lap_s_count, 0);
    EXPECT_GE(lap_ms_count, 80);
    EXPECT_GE(lap_us_count, 80'000);
    EXPECT_GE(lap_ns_count, 80'000'000);

    // Verify the relationships between different units
    EXPECT_GT(lap_ns_count, lap_us_count * 500) << "ns should be ~1000x us";
    EXPECT_GT(lap_us_count, lap_ms_count * 500) << "us should be ~1000x ms";
    EXPECT_GE(lap_ms_count, lap_s_count * 500) << "ms should be ~1000x s";
}
...
TEST_F(TimeThisTest, LapWithMultipleTimers)
{
    ...
    // Sample younger timer2 first, then older timer1, so any scheduling delay
    // widens timer1's lead rather than narrowing it.
...
TEST_F(TimeThisTest, LapMicrosecondsPrecision)
{
    ...
    EXPECT_GE(lap_us_count, 80'000) << "lap<microseconds>() should be at least 80,000us";
    EXPECT_LE(lap_us_count, 5'000'000) << "lap<microseconds>() should be reasonable (<5s)";
}
...
TEST_F(TimeThisTest, LapMillisecondsSecondsComparison)
{
    ...
    // Sample seconds first, then milliseconds
    const auto lap_s  = timer.lap<seconds>();
    const auto lap_ms = timer.lap<milliseconds>();
